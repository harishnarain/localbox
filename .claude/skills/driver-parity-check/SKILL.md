...
description: Verify a change to internal/drivers/{macos,linux,windows}/ has been mirrored across all three platform drivers, or has an explicit, tracked deferral. Use before merging any driver change, per CLAUDE.md Principle 4.
---

# Driver parity check

CLAUDE.md Principle 4: a feature isn't "done" on one platform driver — it's
done on all three, or parity is explicitly deferred with a tracked issue.
...
2. For each of the other two platforms, check whether the equivalent
...
3. If the change is to the *shared driver interface* itself (not a single
   platform's implementation), all three drivers must compile against it;
   confirm none were left implementing a stale version of the interface.
